defmodule Breeze.View do
  ...
  * `implicit` - this is a module that will be used for implicit state. This is covered
  ...
  @type slot_value :: iodata() | list()
  @type slot_entry :: %{optional(atom()) => term(), __breeze_slot__: (map() -> slot_value())}
  ...
  """
  @spec render_slot(nil | slot_entry() | [slot_entry()]) :: slot_value()
  def render_slot(slot), do: render_slot(slot, %{})

  @doc """
  Render a slot value with assigns passed to the slot render function.
  """
  @spec render_slot(nil | slot_entry() | [slot_entry()], map() | keyword() | nil) :: slot_value()
  def render_slot(nil, _), do: ""

  def render_slot(slots, assigns) when is_list(slots) do
    rendered = Enum.map(slots, &render_slot(&1, assigns))

    if Enum.all?(rendered, &tree_nodes?/1) do
      Enum.flat_map(rendered, & &1)
    else
      Enum.map_join(rendered, "", &slot_to_binary/1)
    end
  end
  ...
  defp slot_to_binary(value) when is_binary(value), do: value
  defp slot_to_binary(value) when is_list(value), do: IO.iodata_to_binary(value)
  defp slot_to_binary(value), do: to_string(value)
